...
Compose 设置 `MAX_UPLOAD_MB=1024`，即单文件 1 GiB。访客限制为最多 5 次上传、每个视频最多 60 秒、共 5 次 AI 操作；生成、重新配音、字幕校准、人物识别和 AI 改写共用次数。注册账号采用部署配置的时长限制，当前没有 AI 总次数上限。

前端将文件分成不超过 8 MiB 的请求，Caddy 保留 `max_size 1GB` 请求上限与 150 秒读写超时。文件总大小由后端校验；生成任务异步运行，生成文件就绪后下载，不要求一个 HTTP 请求等待整个处理过程。
...
- 打开实际域名的 HTTPS 地址，证书应有效，首次打开显示英文介绍页，并提供语言切换、登录及体验入口。
- 使用两个独立浏览器会话：分别创建账号或体验，只能看到各自上传的素材。复制一个会话的成片链接到另一个会话应返回 404，退出后应返回 401。
- 上传一个合成或明确授权的短视频，确认分块上传、生成进度、播放及导出正常。不要使用维护者的私人案例做公开演示。
- 核对访客 5 次上传、1 GiB / 60 秒及 5 次 AI 额度。生成完成后可直接导出，审校和质量提示不应成为下载门禁。
