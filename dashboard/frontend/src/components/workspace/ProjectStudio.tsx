...
export default function ProjectStudio({ projectId, processingReady, speechReady = processingReady, speechLanguages = defaultSpeechLanguages, initialIntent, backLabel, onBack, onSetup }: { projectId: string; processingReady: boolean; speechReady?: boolean; speechLanguages?: SpeechLanguage[]; initialIntent?: StudioIntent; backLabel?: string; onBack: () => void; onSetup: () => void }) {
  ...
  const stepLabels: Record<string, string> = {
    DetectCharacters: t("识别人物外观", "Detecting character appearances"), SilenceDetection: t("寻找解说时间", "Finding narration windows"), ValidateInput: t("检查视频", "Checking video"), TranscribeVideo: t("识别对白", "Transcribing dialogue"), ExtractSilenceSegments: t("寻找解说时间", "Finding narration windows"), DetectSilence: t("寻找解说时间", "Finding narration windows"),
  ...
  const busy = running || calibrating || reconciling || auxiliarySaving;
  ...
  const guard = (action: () => void) => { if (auxiliarySaving || reconciling) return; if (dirty) setDiscardAction(() => action); else action(); };
  ...
  const createCharacter = (frame: CharacterFrameSelection) => characterRef.current?.createFromFrame(frame);
  ...
  const refreshDetail = useCallback(async (selectNew = false) => {
    ...
    setDetail(data);
    if (!characterDetectionSelectionInitialized.current && typeof data.executions[0]?.detect_characters === 'boolean') { setDetectCharacters(data.executions[0].detect_characters); characterDetectionSelectionInitialized.current = true; }
  ...
  return <main className="ws-studio" id="visionecho-content" tabIndex={-1}>
    ...
    <div className="ws-studio-grid"><div className="ws-video-column">
      ...
      <CharacterPanel key={projectId} refreshKey={versionReload} automaticDetection={editor?.character_detection} ref={characterRef} projectId={projectId} jobId={resultId} segments={segments} disabled={busy || starting || saving || editorLoading || detail.project.archived} onDirtyChange={setCharacterDirty} onBusyChange={setCharacterSaving} onApply={applyCharacterNames} />
      <section className="ws-settings-panel ws-studio-target" ref={settingsRef} tabIndex={-1} aria-label={t("生成设置", "Generation settings")}>
        ...
        <label className="ve-detect-setting"><input type="checkbox" checked={detectCharacters} disabled={busy || starting || detail.project.archived} onChange={event => { characterDetectionSelectionInitialized.current = true; setDetectCharacters(event.target.checked); }} />{t("自动识别人物", "Detect characters automatically")}<small>{t("生成待确认的人物卡", "Create unconfirmed character cards")}</small></label>
