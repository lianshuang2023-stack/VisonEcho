...
export default function AccessControls({ disabled = false }: { disabled?: boolean }) {
  ...
  return <div className="ve-access-controls" title={disabled ? t('返回作品列表后切换账号', 'Return to the video list to switch accounts') : undefined}>
    ...
    <button type="button" disabled={blocked} onClick={() => openAccess('logout')}><LogOut size={13} />{t('退出', 'Log out')}</button>
  </div>;
}
