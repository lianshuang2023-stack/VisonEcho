...
// The main website address opens the introduction in every access mode.
// The explicit studio anchor keeps ongoing work directly reachable.
...
export default function AccessGate({ children }: { children: ReactNode }) {
  ...
  const [showAbout, setShowAbout] = useState(() => isOverviewHash(window.location.hash));
  ...
  useEffect(() => {
    const updateOverview = () => setShowAbout(isOverviewHash(window.location.hash));
    window.addEventListener('hashchange', updateOverview);
    return () => window.removeEventListener('hashchange', updateOverview);
  }, []);
  useEffect(() => {
    if (showAbout || loadError) workspaceRoot.current?.querySelectorAll<HTMLMediaElement>('video, audio').forEach(media => media.pause());
  }, [showAbout, loadError]);
  ...
  async function authenticate(path: '/guest' | '/login' | '/register' | '/logout', payload: Record<string, string> = {}) {
    ...
    if (path === '/logout') { setSession(null); setShowLogin(false); }
    try {
      const next = await accessRequest(path, payload);
      if (!mounted.current) return;
      setWorkspaceEpoch(value => value + 1); setSession(next); setDialog(null); setLoadError(''); setShowLogin(false);
      if (next.user || next.mode === 'local') { setShowAbout(false); if (isOverviewHash(window.location.hash)) window.location.hash = 'visionecho-content'; }
    } catch (reason) {
      if (!mounted.current) return;
  ...
  const returnToWorkspace = () => { if (busy || loadError) return; setShowAbout(false); window.location.hash = 'visionecho-content'; };

  if (loading) return <div className="ws-app ve-atelier ve-access-shell"><Loading text={t('正在打开工作区…', 'Opening workspace…')} /></div>;
  if (!hasWorkspace && (!showLogin || loadError)) return <LandingPage busy={busy || Boolean(loadError)} error={loadError || error} onRetry={loadError ? () => { setLoading(true); void refresh(); } : undefined} onStart={() => void authenticate('/guest')} onLogin={() => { if (busy || loadError) return; setError(''); setFormMode('login'); setShowLogin(true); }} />;
  if (!hasWorkspace) return <div className="ws-app ve-atelier ve-access-shell">
    <header className="ve-access-header"><a className="ws-brand" href="#about" aria-label="VisionEcho" title={t('返回项目介绍', 'About VisionEcho')} onClick={() => { setShowLogin(false); setError(''); }}><Clapperboard size={24} /><span>VisionEcho</span></a><div className="ve-header-controls"><button className="ve-preference-button" aria-label={t('切换页面为英文', 'Switch interface to Chinese')} onClick={() => setLanguage(language === 'en' ? 'zh-CN' : 'en')}><Languages size={17} />{language === 'en' ? '中文' : 'EN'}</button><button className="ve-preference-button" aria-label={theme === 'light' ? t('切换到暗色', 'Switch to dark theme') : t('切换到亮色', 'Switch to light theme')} onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')}>{theme === 'light' ? <Moon size={17} /> : <Sun size={17} />}</button></div></header>
    <main className="ve-access-card"><button type="button" className="ws-text-button" disabled={busy} onClick={() => { setShowLogin(false); setError(''); }}>{t('返回项目介绍', 'Back to overview')}</button><div className="ve-access-title"><Clapperboard size={28} /><h1>{t('口述电影工作台', 'Audio description studio')}</h1><p>{t('上传视频，制作属于你的口述作品。', 'Upload a video and create your audio-described version.')}</p></div>
  ...
  return <AccessSessionContext.Provider value={{ session: session!, busy: busy || Boolean(loadError), openAccess, refresh }}>
    <div ref={workspaceRoot} hidden={Boolean(loadError) || showAbout} inert={Boolean(loadError) || showAbout}><Fragment key={workspaceKey}>{children}</Fragment></div>
    {showAbout && <LandingPage hasWorkspace busy={busy || Boolean(loadError)} error={loadError} onRetry={loadError ? () => void refresh() : undefined} onStart={returnToWorkspace} onLogin={returnToWorkspace} />}
    {loadError && !showAbout && <div className="ws-app ve-atelier ve-access-shell"><main className="ve-access-card"><h1>{t('工作区连接暂时中断', 'Workspace connection interrupted')}</h1><p className="ve-access-note">{t('当前草稿已保留。重新验证会话后继续编辑。', 'Your current drafts are retained. Verify the session again to continue editing.')}</p><Alert>{loadError}</Alert><button className="ws-button secondary" disabled={busy} onClick={() => void refresh()}>{t('重新检查连接', 'Check connection again')}</button></main></div>}
    {dialog && !loadError && !showAbout && <div className="ws-app ve-atelier ve-access-modal"><Modal title={dialog === 'logout' ? t('退出工作区', 'Log out of workspace') : dialog === 'register' ? t('注册保存作品', 'Register to keep videos') : t('登录账号', 'Log in to an account')} busy={busy} onClose={() => setDialog(null)}>
